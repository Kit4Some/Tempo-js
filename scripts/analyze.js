...
//   - Mann-Whitney U (non-parametric two-sided) — asymptotic normal
//     approximation with continuity correction and tie-corrected variance.
//   - Cohen's d effect size with pooled SD (Cohen 1988).
//   - 1000-resample percentile bootstrap for 95% CIs (seeded RNG for
//     reproducibility — BOOT_SEED below).
//   - Go/No-Go gate: p < 0.05 AND |d| ≥ 0.5 on jankRate versus B1.
//
// Failure policy: if status='error' rate exceeds 5%, the verdict section
// is suppressed and a WARNING section takes its place — do not publish
// a verdict on a half-broken run.
...
function renderVerdictSummary(perWorkload, failureRate) {
  if (failureRate > MAX_FAILURE_RATE) {
    return [
      "## ⚠ Verdict withheld",
      "",
      `Failure rate ${(failureRate * 100).toFixed(2)}% exceeds the ${(MAX_FAILURE_RATE * 100).toFixed(0)}% ceiling. Verdict suppressed — investigate the failed runs before re-running analysis.`,
      "",
 ...
 * decision is left to human review.
 */
...
function main() {
  ...
  const header = [
    ...
    "Statistical methodology: Mann-Whitney U (asymptotic, continuity-corrected, tie-adjusted), Cohen's d (pooled SD), 1000-resample seeded bootstrap. Gate: p < 0.05 AND |d| ≥ 0.5.",
    "",
  ].join("\n");
